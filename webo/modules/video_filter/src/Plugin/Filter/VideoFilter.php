...
class VideoFilter extends FilterBase implements ContainerInjectionInterface {
   ...
   *
   * @deprecated in video_filter:8.x-1.1 and is removed from video_filter:2.0.0. Use $this->pluginManager instead.
   *
   * @see https://www.drupal.org/node/3307576
   */
  ...
  public function __construct(array $configuration, $plugin_id, $plugin_definition) {
    ...
    $this->pluginManager = \Drupal::service('plugin.manager.video_filter');
    // Backwards compatible.
    $this->plugin_manager = $this->pluginManager;
  }
    ...
    if (preg_match_all('/\[video(\:(.+))?( .+)?\]/isU', $text, $matches_code)) {
      ...
      foreach ($matches_code[0] as $ci => $code) {
        ...
        if (isset($video['codec'])) {
          ...
          if (!empty($video['control_bar_height']) && is_numeric($video['control_bar_height'])) {
            // Respect control_bar_height option if present.
            $control_bar_height = intval($video['control_bar_height']);
          }
          elseif (!empty($video['codec']['control_bar_height']) && is_numeric($video['codec']['control_bar_height'])) {
            // Respect setting provided by codec otherwise.
            $control_bar_height = intval($video['codec']['control_bar_height']);
          }

          // Resize to fit within width and height repecting aspect ratio.
          if ($ratio && is_numeric($video['height']) && is_numeric($control_bar_height)) {
            $scale_factor = min([
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    $form['width'] = [
      ...
      '#description' => $this->t('Default width value. Can be overridden on each video through options on the shortcode. If not provided no width value will be added to the HTML element, allowing the size to be controlled using CSS.'),
    ...
    $form['height'] = [
      ...
      '#description' => $this->t('Default height value. Can be overridden on each video through options on the shortcode. If not provided no height value will be added to the HTML element, allowing the size to be controlled using CSS.'),
    ...
    $plugins = [];
    foreach ($this->pluginManager->getDefinitions() as $plugin_info) {
      $plugin = $this->pluginManager->createInstance($plugin_info['id']);
      $plugins[$plugin_info['id']] = $plugin->getName();
    }
    // Sort the plugins before displaying them.
    asort($plugins);
    $form['plugins'] = [
      '#type' => 'checkboxes',
